...
public class OWLQN {
	...
	*/
	...
	public double[] minimize(DiffFunction function, double[] initial, double l1weight, double tol, int m) {
		...
		for(int i=0; i<maxIters; i++){
			...
			if(arrayEquals(state.x, state.newX)){
				System.out.println("Warning: Stopping OWL-QN since there was no change in the parameters in the last " +
	...
	/** Projects the full weight vector into the feasible region where constrains are operative only on a
     * subset of weights.
...
class OptimizerState {
	...
	private void printStateValues() {
		...
		for(int i=0; i<sList.size(); i++){
			System.out.println(arrayToString(sList.get(i)));
		}
		
		System.out.println("YLIST:");
		for(int i=0; i<yList.size(); i++){
			System.out.println(arrayToString(yList.get(i)));
		}
		
		System.out.println("ROLIST:");
		for(int i=0; i<roList.size(); i++){
			System.out.println(roList.get(i));
		}
		System.out.println();
	}
	...
	void testDirDeriv() {
		...
		//if (!quiet) System.out.print("  Grad check: " + numDeriv + " vs. " + deriv + "  ");
	}
	 */
